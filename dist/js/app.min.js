...
angular.module("app", [ "app.core", "app.flickr", "ui.router", "ui.bootstrap", "ngSanitize", "ngTouch", "ngResource", "infinite-scroll", "angularFileUpload", "ngImgCrop" ]).run([ "$rootScope", "$state", "$stateParams", "$window", "$location", "$timeout", function($rootScope, $state, $stateParams) {
...
angular.module("app.core", [ "ui.router", "angularFileUpload" ]).config([ "$stateProvider", "$urlRouterProvider", function($stateProvider) {
    ...
    $stateProvider.state("modal.flickr", {
        ...
        controller: function($scope, $rootScope, $state, Flickr) {
            $scope.filters = {
            ...
            }), $scope.selected = [], $scope.items = [], $scope.active = void 0, $scope.loadItems = function(push) {
                Flickr.load($scope.filters, function(data) {
                    ...
                    }), void 0 !== push && 1 == push ? Array.prototype.push.apply($scope.items, structured) : $scope.items = structured;
                });
            }, $scope.updateFilters = function() {
                ...
                void 0 != item && ($rootScope.multiple && item.active ? (item.active = !1, angular.forEach($scope.selected, function(activeItem, key) {
                    ...
                    $scope.active.name = data.photo.title._content, $scope.active.link = data.photo.urls.url[0]._content, 
    ...
    $scope.init = function(params) {
        ...
        $rootScope.files = void 0 != params.files ? params.files : [];
    };
    $scope.uploader = new FileUploader({
        url: $rootScope.apiUrlUpload + "upload",
        autoUpload: !0
